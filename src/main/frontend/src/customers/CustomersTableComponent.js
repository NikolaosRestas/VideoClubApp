...
export default function CustomersTableComponent({ customers, onChange }) {
    ...
    function onDelete(customer) {
        fetch(`/customers/delete/${customer.id}`, {
            ...
            .catch(error => {
                setErrorMessage(`Error: Could not delete ${customer.name} because it is a key to other Entities. Try and delete those first.`);
    ...
    return (
        <>
            <TableContainer component={Paper} className="shadow-lg rounded-lg">
                <Table className="min-w-max" aria-label="customer table">
                    <TableHead>
                        <TableRow>
                            <TableCell className="font-bold">Id</TableCell>
                            <TableCell align="right" className="font-bold">Name</TableCell>
                    ...
                    <TableBody>
                        {customers.map((customer) => (
                            <TableRow key={customer.id} className="hover:bg-gray-100">
                                <TableCell className="py-3">{customer.id}</TableCell>
                                <TableCell align="right" className="py-3">{customer.name}</TableCell>
                                <TableCell align="right" className="py-3">{customer.phone}</TableCell>
                                <TableCell align="right" className="py-3">{customer.email}</TableCell>
                                <TableCell align="right" className="py-3">{customer.videoClub.name}</TableCell>
            ...
            {isSuccessfulDelete && (
                <div className="relative h-32 flex justify-center">
                    <Alert severity="success">
                        The customer <strong>{isSuccessfulDelete.customerName}</strong> was deleted successfully!
                    </Alert>
                </div>
            )}
