...
export default function EditCdModal({isOpen, onClose, clientData, onSave}) {
    ...
    const handleSave = () => {
        ...
        clientData.artist = editedData.artist;
    ...
    return (
        <React.Fragment>
            <Dialog open={isOpen} onClose={onClose}>
                <DialogTitle>Edit Cd</DialogTitle>
                <DialogContent>
                    <TextField
                        label="Name"
                        name="name"
            ...
            <div className="relative h-32 flex flex-nowrap">
                <div className="absolute inset-x-0 bottom-0 h-16 flex flex-nowrap">
                    {isSuccessAlertOpen && <Alert severity="success">The Cd update was successful!</Alert>}
                </div>
            </div>
        </React.Fragment>
    );
