...
export default function EditVideoclubModal({isOpen, onClose, clientData, onSave}) {
    ...
    return (
        <React.Fragment>
            <Dialog open={isOpen} onClose={onClose}>
                <DialogTitle>Edit Videoclub</DialogTitle>
                <DialogContent>
                    <TextField
                        label="Name"
                        name="name"
            ...
            <div className="relative h-32 flex flex-nowrap">
                <div className="absolute inset-x-0 bottom-0 h-16 flex flex-nowrap">
                    {isSuccessAlertOpen && <Alert severity="success">The videoclub update was successful!</Alert>}
                </div>
            </div>
        </React.Fragment>
    );
